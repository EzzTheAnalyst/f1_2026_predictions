...
"""
Austria GP 2026 - Winner Prediction Model
===========================================


Approach: Train on Free Practice (FP2,FP3,Q)
Reasoning:
...
for session_name in ["FP2", "FP3", "Q"]:
    session = fastf1.get_session(2026, "Austria", session_name)
    session.load()
...
df_fp["race_pace_median"] = df_fp[["FP2_median","FP3_median"]].mean(axis=1)
...
API_KEY = ""
City = "Austria"
date_of_race = "2026-06-28"                     # must follow YYYY-MM-DD
...
for col in ["FP2_median", "FP3_median", "race_pace_median"]:
    merged_data[col] = merged_data[col].fillna(merged_data["QualifyingTime (s)"] + 1.0)

# define features (x) and target (y)
X = merged_data[["QualifyingTime (s)", "FP2", "FP3", "race_pace_median"]]
